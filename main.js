...
global.authFile = `Spidermansession`;
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['spidermanbot-MD', 'Safari', '2.0.0'] : methodCodeQR ? ['itachixov-Bot-MD', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '110.0.5585.95'],
auth: {
creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./DK3MK")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./DK3MK/${files}`)
})
} 
...
const directories = ['./DK3MK/', './jadibts/']
...
if (reason == 405) {
await fs.unlinkSync("./DK3MK/" + "creds.json")
...
global.reloadHandler = async function(restatConn) {
  ...
  conn.bye = '*╭┈⊰* @subject *⊰┈ ✦*\n┊ *المستخدم* @user\n┊ *غادر المجموعة حديثاً 🐸*\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*'
  conn.spromote = '*@user أنت الآن مشرف في هذه المجموعة!!*'
  conn.sdemote = '*@user لم تعد  مشرفا في هذه المجموعة!!*'
  conn.sDesc = '*تم تعديل وصف المجمووعه الوصف الجديد @desc*'
  conn.sSubject = '*تم تغير اسم الجروب \n الاسم الجديد @subject'
  conn.sIcon = '*تم تغير صوره الجروب بواسطه @user *';
...
setInterval(async () => {
  ...
  const bio = `[ 🕸️ ] 𝑺𝑃𝐼𝐷𝛯𝑅𝑀𝐴𝑁𝐵𝛩𝑇-𝑀𝐷: ${uptime}`;
...
function clockString(ms) {
  ...
  return [d, 'يـوم ️', h, 'سـاعه ', m, 'دقـيـقـه ', s, 'ثـانـيـه '].map((v) => v.toString().padStart(2, 0)).join('');
}
